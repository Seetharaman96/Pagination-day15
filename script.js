...
function childClick(e){
    ...
    var getData = parseInt(e.getAttribute('for'));

    var syncData = datas[getData-1];

    var creatingDiv = document.createElement('div');

    creatingDiv.innerHTML = `
        <p for="id">Id: ${syncData.id}</p>

        <p for="name">Name: ${syncData.name}</p>
        
        <p for="email">Email: ${syncData.email}</p>
    `
...
btnNext.addEventListener('click',()=>{
    ...
    for(i=1;i<=5;i++){
        ...
        var firstPage = document.querySelector(`.btn-${i}`)

        var attributeVal = parseInt(firstPage.getAttribute('for'))

        if (attributeVal <= 95){

            var increasingVal = attributeVal + 5;

            firstPage.setAttribute('for',increasingVal);

            firstPage.innerText = increasingVal;
